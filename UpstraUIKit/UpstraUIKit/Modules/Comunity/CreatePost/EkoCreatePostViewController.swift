//
//  EkoCreatePostViewController.swift
//  UpstraUIKit
//
...
enum EkoPostType {
    case user
    case comunity(EkoCommunity)
}

public class EkoCreatePostViewController: EkoViewController {
    ...
    var role: EkoPostType = .user {
        didSet {
            updateVewLayout()
        }
    }
    
    var postMode: EkoPostMode = .create {
        didSet {
            postModeDidSet()
        }
    }
...
extension EkoCreatePostViewController: EkoCreatePostMenuViewDelegate {
    ...
    func postMenuView(_ view: EkoCreatePostMenuView, didTap action: EkoPostMenuActionType) {
        switch action {
        case .camera:
            break
        case .photo:
