//
//  EkoCommentChildrenController.swift
//  UpstraUIKit
//
...
final class EkoCommentChildrenController {
    
    private let postId: String
    private let commentRepository: EkoCommentRepository = EkoCommentRepository(client: UpstraUIKitManagerInternal.shared.client)
    ...
    private var commentChildrenCollections: [String: EkoCollection<EkoComment>] = [:]
    private var commentChildrenTokens: [String: EkoNotificationToken] = [:]
    private var commentChildrenResults: [String: [EkoCommentModel]] = [:]
    
    func childrenComments(for parentId: String) -> [EkoCommentModel] {
    ...
    func fetchChildren(for parentId: String, completion: (() -> Void)?) {
        if let collection = commentChildrenCollections[parentId] {
            ...
            commentChildrenCollections[parentId] = commentRepository.comments(withReferenceId: postId, referenceType: .post, filterByParentId: true, parentId: parentId, orderBy: .descending, includeDeleted: true)
            commentChildrenTokens[parentId] = commentChildrenCollections[parentId]?.observe { [weak self] collection, _, _ in
                guard let strongSelf = self else { return }
                var commentModels: [EkoCommentModel] = []
                for i in 0..<collection.count() {
                    guard let childComment = collection.object(at: i) else { continue }
                    commentModels.append(EkoCommentModel(comment: childComment))
                }
