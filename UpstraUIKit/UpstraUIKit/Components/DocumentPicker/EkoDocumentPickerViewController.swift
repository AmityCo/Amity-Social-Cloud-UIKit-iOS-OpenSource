//
//  EkoDocumentPickerViewController.swift
//  UpstraUIKit
//
...
protocol EkoFilePickerDelegate: class {
    func didPickFiles(files: [EkoFile])
}

public class EkoDocument: UIDocument {
...
open class EkoFilePicker: NSObject {
    private var pickerController: UIDocumentPickerViewController?
    private weak var presentationController: UIViewController?
    private weak var delegate: EkoFilePickerDelegate?
    private var folderURL: URL?
    private var files = [EkoFile]()
    ...
    init(presentationController: UIViewController, delegate: EkoFilePickerDelegate) {
        super.init()
    ...
    public func present(from sourceView: UIView, files: [EkoFile]) {
...
extension EkoFilePicker: UIDocumentPickerDelegate{
    ...
    private func documentFromURL(pickedURL: URL) {
        NSFileCoordinator().coordinate(readingItemAt: pickedURL, error: NSErrorPointer.none) { (folderURL) in
            ...
            let document = EkoDocument(fileURL: folderURL)
            let file = EkoFile(state: .local(document: document))
            files.append(file)
        }
...
extension EkoFilePicker: UINavigationControllerDelegate {}
