...
extension UIColor {
    ...
    func blend(_ option: ColorBlendingOption) -> UIColor {
        ...
        let key = EkoColorCache.shared.key(for: self, lum: luminant)
        
        if let color = EkoColorCache.shared.getColor(key: key) {
            return color
        } else {
            var hslColor = rgbToHsl()
            hslColor.lum += option.rawValue
            let blendedColor = hslToRgb(h: hslColor.hue, s: hslColor.sat, l: hslColor.lum)
            EkoColorCache.shared.setColor(key: key, color: blendedColor)
            return blendedColor
        }
    }
...
private class EkoColorCache {
    
    private let cache = NSCache<NSString, UIColor>()
    static let shared = EkoColorCache()
